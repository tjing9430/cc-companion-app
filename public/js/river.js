...
// 这张表对应 assets/galaxy-river.webp。换图必换它。
const RIVER = [
  [0.00, 0.742, 0.100, 0.062],
  [0.10, 0.811, 0.184, 0.085],
  [0.20, 0.736, 0.268, 0.104],
  [0.30, 0.519, 0.352, 0.120],   // ← half 原始量到 0.216,是**折返处两股被当成一股**量出来的:
                                 //    S 形在这儿掉头,同一行有两条带子,提取器把中间的空隙也算进了带宽。
                                 //    中位数才 0.09,这个值是异常点,按上限 0.12 收 —— 不收的话星星会被推得太靠外,
                                 //    外面就留不出放文字的地方了。
  [0.40, 0.440, 0.436, 0.087],
  [0.50, 0.545, 0.520, 0.105],
  [0.60, 0.529, 0.604, 0.095],
  [0.70, 0.387, 0.688, 0.088],
  [0.80, 0.379, 0.772, 0.106],
  [0.90, 0.398, 0.856, 0.090],
  [1.00, 0.221, 0.940, 0.081],
];
...
function layoutGates(gates) {
  ...
  return list.map((g, i) => {
    const t = spreadT(n, i);
    const p = riverAt(t);
    const starHalf = (Number(g.size) || 0) / 200;
...
export { RIVER, T_START, T_END, MAX_GATES, VISIBLE, TEXT_ROOM, riverAt, spreadT, layoutGates };
