...
// ★ 8/14 重摆:她的反馈是「各个小岛摆的不好看」。对照她给的目标图,差在**节奏**:
//   目标图是一条匀速的蛇形——相邻两座竖直间距一致、左右开满。旧表 y 间距 13/11/12/9 忽紧忽松,
//   底部三座(设置/更多/标签)挤成一团。新表:y 等距 11.5,一路从 33 走到 79,「更多」86.5 收尾。
//   x 不往中间收——六座里有两座横岛占容器宽 34%,再收就要撞(拿四档盒子验过,不是手感)。
// ★ 8/14 16:28 她看完 v1 的追改:「可以往两边放一点 这些小岛」——
//   左列 32/33→28、右列 66/67→73/74,往可见区两缘推。
//   边界不是拍的:可见区 [10,90] 是 360×800 档(她那台就是这档)的最狠裁切,
//   推到的位置留了余量——最挤的「群聊」(横岛,半宽 17)左缘落在 11%,
...
// ★ side below → right 是 8/14 重摆的一部分:下方标签是旧版底部越界(y 91/88 两次红测)
//   的根源——竖向多吃一整条标签高。挪到旁边后岛可以沉到 86.5,和「设置」(y79 右侧)
//   一左一右收尾,正好是目标图里热气球平台在云海里的位置。
// ★ 「标签压在岛上」**不在约束里** —— 需求方的界面稿上本来就是白药丸压着岛边。
//   我第一版把它当硬规矩,768 组配置全军覆没,毙掉它们的是我发明的规矩。
// ★ y 86.5 → 88.5(8/14 下午):拿**真行高 1.6** 重跑判据,320×568 上「更多」标签
