...
if (process.env.CC_SKIP_DOTENV !== '1') loadDotEnv(path.join(ROOT_DIR, '.env'));
...
function loadStoreSqlite() {
  ...
  if (sqliteStore.isEmpty()) {
    ...
    if (fs.existsSync(STORE_FILE)) {
      ...
      // 全新安装(#70②):走和 JSON 后端同一条出生路径。原来这里落到
      // normalizeStore(loadAll()) —— 把「空」洗成 defaults 形状,而 defaults 的
      // companion_since 是 ''(那是给老库升级的语义),defaultStore() 里
      // 「首跑=今天」的起算戳在 sqlite 后端永远没人落。种完立刻持久化,
      // 下次启动才不会又被当空库再种一遍。
      try {
        const fresh = defaultStore();
        sqliteStore.replaceAll(fresh);
        return fresh;
      } catch (err) {
        console.warn(`Could not seed fresh sqlite store (${err.message}); falling back to JSON backend.`);
        sqliteStore.close(); sqliteStore = null;
        return loadStoreJson();
      }
    }
...
function saveStoreSqlite(hint) {
  ...
  // 删单条走行级删除(#70③):走兜底 replaceAll 的话,被删消息的**回复**还带着它的
  // parent_msg_id,重插整表在 REFERENCES 上直接 787(constraint failed)。行级删除让
  // ON DELETE SET NULL 在库内把子行熨平;内存里子行保留悬空引用是设计内(前端判空跳过)。
  if (k === 'message_delete' && hint.id != null) return sqliteStore.tx(() => sqliteStore.deleteMessage(hint.id));
...
function normalizeSettings(input) {
  ...
  return {
    ...
    theme: ['light', 'starry', 'island'].includes(settings.theme) ? settings.theme : 'dark',
    ...
    user_signature: cleanString(settings.user_signature, '').slice(0, 80),
    assistant_signature: cleanString(settings.assistant_signature, '').slice(0, 80),
    session_max_tokens_k: Math.min(960, Math.max(32, Math.round(Number(settings.session_max_tokens_k) || 600))),
  };
}
...
function agentStatus() {
  const dsh = String(process.env.AGENT_PROVIDER || '').toLowerCase() === 'dsh';
  return {
    provider: dsh ? 'deepseek-harness' : (process.env.OPENAI_API_KEY ? 'openai-compatible' : 'mock'),
    model: dsh ? (process.env.DSH_MODEL || 'deepseek-v4-flash') : (process.env.OPENAI_API_KEY ? (process.env.OPENAI_MODEL || 'gpt-4.1-mini') : 'mock-agent'),
