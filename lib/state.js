...
// 存储后端。默认仍是 JSON —— 这是**封笔版的风险姿势**:
// 新路径错了她一点事没有,而 §6 定的回滚(「删掉 .db 继续用 JSON」)也只有
...
function saveStoreSqlite(hint) {
  ...
  if (k === 'message' && hint.row) return sqliteStore.putMessage(hint.row);
  if (k === 'console' && hint.row) return sqliteStore.putConsoleEvent(hint.row, 500);
  if (k === 'memory' && hint.row) return sqliteStore.putMemory(hint.row);
  if (k === 'document' && hint.row) return sqliteStore.putDocument(hint.row);
