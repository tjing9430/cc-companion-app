...
function normalizeApiUsage(input) {
  ...
  const token = (value) => Math.max(0, Math.floor(Number(value) || 0));
  const usage = {
    provider: cleanString(input.provider, 'api').slice(0, 20),
    ...
    reasoning_tokens: token(input.reasoning_tokens),
  };
  return usage.input_tokens || usage.output_tokens || usage.cache_read_tokens || usage.reasoning_tokens ? usage : null;
}
...
function addMessage(scope, input) {
  ...
  const message = {
    ...
    api_usage: normalizeApiUsage(input.api_usage),
    ...
    created_at: cleanString(input.created_at, new Date().toISOString()),
  };
