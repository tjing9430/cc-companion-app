...
async function forgeSession() {
  ...
  chat.rows.push(createForgeMarker('chat', nextIdValue, now, external, chat.rows));
  group.rows.push(createForgeMarker('group', nextIdValue, now, external, group.rows));
...
function createForgeMarker(scope, nextIdValue, now, external, rows = []) {
  const externalDone = external && external.configured;
  const excerpts = rows.slice(-12).map((message) => {
    const at = cleanString(message.created_at, '').replace('T', ' ').slice(0, 16);
    const speaker = message.role === 'assistant'
      ? (store.settings.assistantName || 'AI')
      : (message.sender || store.settings.userName || '用户');
    return `[${at || '时间未知'}] ${speaker}：${truncate(cleanString(message.content, ''), 220)}`;
  }).filter(Boolean);
  const handoff = excerpts.length
    ? `上一段 Session 的交接纸条（仅保留聊天原文，不含 thinking、tool 调用）：\n${excerpts.join('\n')}`
    : '上一段 Session 没有可交接的聊天原文。';
  return {
    id: nextId('message'),
    scope,
    sender: '系统',
    role: 'assistant',
    content: `${handoff}\n\n${externalDone ? '外部 agent session 已同步更换。' : '已开启新的本地/DSH session。'}`,
    attachments: [],
    parent_msg_id: null,
    msg_type: 'forge',
    session_id: nextIdValue,
