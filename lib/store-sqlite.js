...
class SqliteStore {
  constructor(db) {
    ...
    this.q = {
      ...
      hasMsg: db.prepare('SELECT 1 FROM messages WHERE id = ?'),
  ...
  loadAll() {
    ...
    // 表名不拼进 SQL 字符串 —— 每张表各自一条写死的查询,靠 Map 查表而不是模板拼接选中它。
    const MAXOF_SQL = new Map([
      ['messages', 'SELECT COALESCE(MAX(id), 0) AS m FROM messages'],
      ['memories', 'SELECT COALESCE(MAX(id), 0) AS m FROM memories'],
      ['console_events', 'SELECT COALESCE(MAX(id), 0) AS m FROM console_events'],
      ['documents', 'SELECT COALESCE(MAX(id), 0) AS m FROM documents'],
      ['stickers', 'SELECT COALESCE(MAX(id), 0) AS m FROM stickers'],
    ]);
    const maxOf = (t) => {
      const sql = MAXOF_SQL.get(t);
      if (!sql) throw new Error(`maxOf: 非法表名 ${t}`);
      return int(this.db.prepare(sql).get().m, 0);
    };
  ...
  putMessage(m) {
    // 悬空的 parent_msg_id 落 NULL,不落违约值(#70③ 的根):内存模型允许「被回复的那条已删」
    // (前端 renderQuotedParent 判空跳过),但 schema 的 REFERENCES 在 INSERT 时会直接 787。
    // 删一条被引用过的消息后走到任何全量重写,重插子行就炸 —— 插入时按「父行此刻在不在」清洗。
    // ★ 依赖插入顺序:父行 id 更小、先插(replaceAll 按数组序=时间序)。乱序插会把活父洗成 NULL。
    const parent = m.parent_msg_id == null ? null
      : (this.q.hasMsg.get(int(m.parent_msg_id)) ? int(m.parent_msg_id) : null);
    this.q.insMsg.run(int(m.id), str(m.scope, 'chat'), str(m.sender, 'unknown'),
      ...
      str(m.msg_type, 'chat'), str(m.session_id), parent,
  ...
  replaceAll(store) {
    this.tx(() => {
      const DELETE_ALL_SQL = new Map([
        ['memory_tags', 'DELETE FROM memory_tags'],
        ['memory_embeddings', 'DELETE FROM memory_embeddings'],
        ['document_chunks', 'DELETE FROM document_chunks'],
        ['messages', 'DELETE FROM messages'],
        ['memories', 'DELETE FROM memories'],
        ['documents', 'DELETE FROM documents'],
        ['console_events', 'DELETE FROM console_events'],
        ['stickers', 'DELETE FROM stickers'],
      ]);
      for (const t of DELETE_ALL_SQL.keys()) this.db.exec(DELETE_ALL_SQL.get(t));
  ...
  checkpoint(mode = 'TRUNCATE') {
    // 模式不拼进 SQL 字符串 —— 每个合法模式各自一条写死的 PRAGMA,靠 Map 查表选中它。
    const CHECKPOINT_SQL = new Map([
    ...
    const sql = CHECKPOINT_SQL.get(mode) || CHECKPOINT_SQL.get('TRUNCATE');
    try { return this.db.prepare(sql).get(); }
