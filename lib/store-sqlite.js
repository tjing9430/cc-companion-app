// SQLite 持久化后端。
//
// 它解决的不是「数据库比较高级」,是一个量出来的具体问题:
// 现在每次 saveStore() 都整文件重写 JSON,而全库有 22 个调用点(每条消息/记忆/日志都触发)。
//   148 KB(实测她当前库)→ 每次 52 ms
//   ~1.5 MB(10 倍,约一年用量)→ 321 ms
//   ~7 MB → 2133 ms
// 也就是说随着用得越久,发一条消息卡得越明显,而且是 O(全库) 不是 O(改动)。
// 换成逐行写之后,追加一条消息就是插一行,和库多大无关。
//
...
class SqliteStore {
  ...
  // ---- 写:每种改动只碰它自己那几行 ----
  putKv(key, value) { this.q.putKv.run(key, JSON.stringify(value)); }
  ...
  close() { try { this.db.close(); } catch { /* 已经关了 */ } }
}
