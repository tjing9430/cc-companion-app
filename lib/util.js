...
function publicMessage(message) {
  ...
  // Forge markers carry a full original-text handoff in storage so the next agent session can
  // continue coherently. That handoff is an internal prompt, not chat copy: expose only a small
  // visible paper note. This also repairs old markers without rewriting conversation history.
  if (out.msg_type === 'forge') {
    out.content = '已整理上一段对话，并开启新的 Session。';
    out.thinking = '';
    out.tools = [];
    out.attachments = [];
  }
