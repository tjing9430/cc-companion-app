...
import { callDshAgent } from './dsh.js';
import { collectAgentFiles } from './agent-files.js';
...
async function handleSend(res, scope, body) {
  ...
  // One composer submit may contain several cached text parts. Give that
  // atomic batch one exact timestamp so the UI can show a single footer on
  // its last bubble without guessing from an arbitrary time window.
...
async function generateAgentReply(scope, userMessage, timing) {
  ...
  let pendingThinking = '';
  let flushTimer = null;
  broadcastSse('message-stream', {
    scope, stream_id: streamId, phase: 'start', sender: assistantName,
    parent_msg_id: userMessage.id, created_at: new Date().toISOString(),
  });
  const flushStream = () => {
    if (flushTimer) { clearTimeout(flushTimer); flushTimer = null; }
    if (!pendingContent && !pendingThinking) return;
    broadcastSse('message-stream', {
      scope, stream_id: streamId, phase: 'delta',
      content: pendingContent, thinking: pendingThinking,
    });
    pendingContent = '';
    pendingThinking = '';
  };
  const onDelta = (channel, delta) => {
    const value = String(delta || '');
    if (!value) return;
    if (channel === 'thinking') { streamedThinking += value; pendingThinking += value; }
    else { streamedContent += value; pendingContent += value; }
    if (!flushTimer) flushTimer = setTimeout(flushStream, 55);
  };
  try {
    const result = await callConfiguredAgent(scope, userMessage, timing, onDelta);
    content = result.content;
    thinking = result.thinking || '';
    tools = Array.isArray(result.tools) ? result.tools : [];
    apiUsage = result.api_usage || null;
    const delivered = collectAgentFiles(content);
    ...
    content = streamedContent
      ? `${streamedContent}\n\n（回复在生成途中断开）`
      : `暂时无法调用已配置的 AI：${err.message}`;
    thinking = streamedThinking;
  }

  flushStream();
  broadcastSse('message-stream', { scope, stream_id: streamId, phase: 'end' });
...
function isLegacyMockTurn(turn) {
  if (!turn || turn.role !== 'assistant') return false;
  const content = String(turn.content || '');
  return content.startsWith('演示 AI 在')
    && content.includes('在 .env 里设置 OPENAI_API_KEY 后，就会切换成真实模型回复。');
}
...
function attachmentContext(userMessage, baseUrl) {
  ...
  // DeepSeek's current OpenAI-compatible chat/reasoner endpoint accepts text
  // message parts only. Sending image_url makes the entire reply fail even
  // though the App has already stored the upload successfully.
  const inlineImages = !/api\.deepseek\.com(?::\d+)?(?:\/|$)/i.test(String(baseUrl || ''));
  const lines = [];
  const parts = [];
  for (const f of files) {
    ...
    lines.push(`- ${label}${isImage(f) && !inlineImages ? '（当前模型不能读取图片内容）' : ''}`);
  }
  return { note: `【用户发来 ${files.length} 个附件】\n${lines.join('\n')}`, parts };
}

async function callConfiguredAgent(scope, userMessage, timing, onDelta = () => {}) {
  if (String(process.env.AGENT_PROVIDER || '').toLowerCase() === 'dsh') {
    tmark(timing, 'recallStart'); tmark(timing, 'recallEnd'); tmark(timing, 'agentStart');
    const history = contextHistory(scope, userMessage.turn_first_id || userMessage.id)
      .filter((turn) => !isLegacyMockTurn(turn));
    try { return await callDshAgent(scope, userMessage, history, onDelta); }
    finally { tmark(timing, 'agentEnd'); }
  }
  ...
  const system = [
    ...
    '请用清楚、简短、自然的中文回复。',
    '如果运行环境允许你创建本机文件，并且用户要求你发送文件，请在最终回复末尾添加 [[CCC_FILE:文件绝对路径]]；CCC 会把该文件转换成附件卡片。',
  ].join('\n\n');
  ...
  let docsBlock = '';
  try {
    // Once a real provider is configured, never feed old built-in demo replies
    // back to it. Otherwise the model can imitate the stale mock wording and
    // appear unconfigured even though the API request succeeded.
  ...
  const requestPayload = {
    ...
    stream: true,
    ...(/api\.deepseek\.com(?::\d+)?(?:\/|$)/i.test(baseUrl)
  ...
  try {
    ...
    if (response.ok) {
      const parsed = await readOpenAiStream(response, onDelta);
      if (parsed.streamed) streamed = parsed;
      else data = parsed.data;
    } else {
  ...
  const text = streamed ? streamed.content : (replyMessage ? replyMessage.content : '');
  ...
  const thinking = streamed ? streamed.thinking
    : (replyMessage ? (replyMessage.reasoning_content || replyMessage.reasoning || '') : '');
  // ★ cc_tools:自带 bridge 用的自定义字段(「用了什么工具、动了哪个文件」)。
  //   普通 OpenAI 兼容口不会有它 —— 缺了就是空数组,前端自然不渲染,不需要开关。
  //   只收 {name, arg} 两个字段并各自截断:这是**别人的服务器**返回的数据,
  //   长度和形状都不该由它说了算。
  const rawTools = streamed && Array.isArray(streamed.tools) ? streamed.tools
    : (replyMessage && Array.isArray(replyMessage.cc_tools) ? replyMessage.cc_tools : []);
  const tools = rawTools.slice(0, 40).map((t) => ({
    name: String((t && t.name) || '').slice(0, 60),
    arg: String((t && t.arg) || '').slice(0, 160),
  })).filter((t) => t.name);
  const rawUsage = streamed ? streamed.usage : (data && data.usage ? data.usage : {});
  const apiUsage = {
    provider: 'api',
    input_tokens: Number(rawUsage.prompt_tokens ?? rawUsage.input_tokens) || 0,
    output_tokens: Number(rawUsage.completion_tokens ?? rawUsage.output_tokens) || 0,
    cache_read_tokens: Number(rawUsage.prompt_cache_hit_tokens ?? rawUsage.cache_read_input_tokens) || 0,
    reasoning_tokens: Number(rawUsage.completion_tokens_details && rawUsage.completion_tokens_details.reasoning_tokens) || 0,
  };
  return {
    content: cleanString(text, mockReply(scope, userMessage)), thinking: cleanString(thinking, ''), tools,
    api_usage: apiUsage,
  };
}
